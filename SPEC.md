...
| D25 | The application mark is drawn from one painter, not shipped as an image | The same geometry serves the launcher icon and the Home glyph on both shells, so there is no second copy to forget to update; the launcher PNGs are rendered from it by `just icons`. → [`D25`](docs/decisions/D25-the-application-mark-is-drawn.md) | `filefin_mark.dart`, `nav_glyph.dart` |
| D26 | A personal access token authenticates through a port, `AuthSession`, not through `SessionManager` | A token has nothing to silently renew with, so its interceptor, its cold-start recovery and its rejection message are all genuinely different from a password session's — the port lets `FileFinClient` wire either without either mode knowing the other exists. → [`D26`](docs/decisions/D26-personal-access-token-auth.md) | `auth_session.dart`, `token_auth_session.dart`, `auth_interceptor.dart` |
