...
**Status:** M0-M5 complete; M6 partial (F10 landed, F5/F6's screens did not — STATE.md). The workspace, every quality gate, the
...
| **M6** | Search, home rows, favourite/rating/watched (F5, F6, F10) | `just check` 0 AND `just it` 0. The POST/DELETE watched distinction proven **behaviourally against the real binary in both directions**: after `POST {"watched":false}` the item is back in `continue` with its pointer intact and the detail offers *Continue*; after `DELETE` it is in no row and offers *Play*. Search exercised live for every field the client can send, including both numeric scopes' fail-closed behaviour. Home rows and search results virtualised on M3's NF2 proxies over a 5000-item list. **PARTIALLY MET — F10 only.** The distinction is measured (M6.0/E-5), proven on the detail screen, and every field is exercised live in M6.0/E-1; the home and search SCREENS and the live suites that would gate them are not built, so the second and third sentences are unmet. STATE.md says which and why |
