...
| **Done** | **M9** — personal access token sign-in against FileFin 0.21.0 (D26): `AuthSession` splits password and token credentials behind one port, `TokenAuthInterceptor` attaches the bearer header and never retries a 401, and the sign-in screen's new mode toggle is proven D-pad reachable. **Deferred**: an in-app token-management UI (create/list/revoke) — this milestone only signs in with a token a user already minted from the server's own Settings page |
