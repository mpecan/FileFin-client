...
| `comments` | `dart_lib_sources` — **tests excluded**. Two checks: a **12-line cap on any one comment block**, applied at every file size, and a **ratio** (35% warn / 45% error) from which files under **20 counted lines** are exempt | test narration is a feature, not debt. A test that says *why* this case exists is doing its job. The block cap is the rule and the ratio is a backstop: measured at M8.R the tree was 37.9% comments — `//` 3.8%, `///` 34.0% — while the gate counted `//` alone and printed zero. Counting both was necessary and not sufficient, because a ratio measures the wrong unit: `transport.dart` carried 71 doc lines over a 12-line function. A comment describing an interface is bounded by that interface; anything longer is a decision (`docs/decisions/`) or a measurement (`docs/field-notes.md`). The size exemption applies to the ratio only — a block is a count, exact at any file size — and every file still exempt and over a line is printed on every run. |
