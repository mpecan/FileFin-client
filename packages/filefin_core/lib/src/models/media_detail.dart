...
/// One sidecar subtitle track — upstream `subtitleInfo`, `media.go`.
///
...
abstract class SubtitleInfo with _$SubtitleInfo {
  /// One sidecar track, every field defaulted.
...
/// bandwidth signal the API gives, and so the whole basis of the metered guard.
///
/// [season] and [episode] are **0** for a single-file item, not null.
@freezed
abstract class FileInfo with _$FileInfo {
  /// One file, every field defaulted.
...
/// A display label and its value — upstream `pair`, `media.go`.
///
/// **[key] is a display label, not a stable identifier.** `orderedPairs`
/// maps known `meta.json` keys through
...
abstract class MetaPair with _$MetaPair {
  /// One display label and its value, both defaulted.
...
/// `media.go`.
///
/// Every list arrives as `[]` and never `null`, and defaults here anyway: the
/// guarantee is upstream's, not ours. A missing `meta.json` is non-fatal, so an
/// item with no metadata is normal rather than an error.
///
/// [continueIndex] and [continueSeconds] are the **derived** resume view rather
/// than the stored pointer, which makes `0`/`0` ambiguous: it is read as "no
/// pointer", which is right for a stale ref and wrong for a real pointer at the
/// very start of a single-file item. [rating] is passed through exactly as
/// sent, including values the write path would reject.
@freezed
abstract class MediaDetail with _$MediaDetail {
  /// The full detail payload, every field defaulted.
