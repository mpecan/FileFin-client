...
///
/// Every interpolated value goes through [_seg]: a parameter containing `/`
...
abstract final class ApiPaths {
  ...
  ///
  /// They are whole segments rather than characters, and Dart's `Uri` deletes
...
class FileFinUrls {
  ...
  ///
  /// The `Uri` constructor rather than `base.replace`, so the base's query and
  /// fragment are dropped: `?theme=dark` must not append itself to every
  /// request. `port` is passed only when [base] wrote one, because `base.port`
  /// otherwise turns `https://h` into `https://h:443`.
  ///
  /// **The two halves filter empties differently, deliberately.** The base half
  /// drops all of them; the path half drops exactly one, the string before
  /// every [ApiPaths] literal's leading `/`. Filtering everywhere quietly did
  /// [ApiPaths._seg]'s job, so an empty segment that gets past `_seg` must
  /// survive into a doubled slash — a visible 404 — rather than vanish.
