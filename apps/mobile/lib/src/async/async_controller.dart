...
class AsyncController<T> extends ChangeNotifier {
  ...
  ///
  /// **Reflecting a change without a full refetch needs exactly this and
  /// nothing more.** The player hands its screen a state folded through the
  /// server's own engine, and re-reading the detail to display it would be the
  /// round trip that fold exists to avoid.
  ///
  /// It does not touch [_token]: the only caller runs from a completed route,
  /// long after `initState`'s load has published, so there is
  ...
  /// Cancels in flight work and refuses to notify afterwards.
  ///
